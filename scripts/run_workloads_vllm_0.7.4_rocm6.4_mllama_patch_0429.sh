...
VLLM_USE_AITER=1 VLLM_USE_AITER_PAGED_ATTN=1 VLLM_USE_TRITON_FLASH_ATTN=0  ../fmwork_vision/driver --model_path /data/llama3.2-90b-vision-instruct --input_size 1024 --output_size 1024  --batch_size 48  --tensor_parallel_size 4  --image_width 256 --image_height 256  --kv_cache_dtype fp8   2>&1 | tee Llama-3.2-90B-Vision-tp4-bs48-kvcache-fp8.txt
...
VLLM_USE_AITER=1 VLLM_USE_AITER_PAGED_ATTN=1 VLLM_USE_TRITON_FLASH_ATTN=0 ../fmwork/infer/vllm/driver --model_path /data/granite-20b-code-instruct-8k --input_size 1024 --output_size 1024 --tensor_parallel 1 --batch_size 80 --dtype bfloat16 --num_scheduler_steps 32 --enable_prefix_caching  --kv_cache_dtype fp8 2>&1 | tee granite-20b-code-bf16-tp1-bs80-aiter-attn-kvcache-fp8.txt
